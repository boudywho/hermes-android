...
| M-005 | High | In progress | Platform | Triage and stage Issue 10 background-execution work (A/B/C phases) | User-enabled foreground monitoring now starts while Hermes is visible, remains continuous across foreground/background transitions, uses Android 14+ special-use classification, is sticky across eligible process restart, retries authenticated session SSE with bounded backoff, and retains fail-closed tray approvals. Remaining scope is device/OEM lifecycle validation and broader cross-client payload/API contract hardening. |
...
| PERF-001 | 2026-08-13 | WebView | Optimized Android WebView prompt typing latency so the document-wide viewport collapse scan no longer runs for ordinary composer/chat attribute churn, while structural and viewport repair triggers remain active. |
