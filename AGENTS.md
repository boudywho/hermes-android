...
- `background/HermesReconnectService.kt` + `background/ReconnectBackgroundPolicy.kt` + `background/ReconnectForegroundServiceTypePolicy.kt` - continuous user-enabled foreground monitor and `/api/session/stream` subscription; Android 14+ uses the declared `specialUse` type and older versions use the declared `dataSync` fallback
...
- A hybrid viewport polyfill is injected because Android WebView computes CSS viewport units (`vh`, `dvh`, etc.) as `0px`. The polyfill injects CSS custom properties (`--vh`, `--dvh`) with measured values, applies baseline CSS for root/layout containers, and uses generic collapse detection outside the primary chat surface without requiring explicit selectors for each WebUI component. Generic detection must also repair fully collapsed (`rect.height <= 0`) shell containers when they hide substantial page-level content (e.g. a `100vh` root collapsing to 0px), otherwise entire pages render blank. The expanded clarification card is a deliberate narrow exception because WebUI mounts its scroller inside `.messages`: repair only `.clarify-card.visible:not(.collapsed)` and its `.clarify-inner` with measured caps, preserving the intentional `.collapsed` dock and the chat-surface anti-flicker exclusion.
...
- Trusted WebUI `blob:` exports use an exact-origin, main-frame AndroidX WebKit document-start/message bridge. Preserve acknowledged bounded chunk streaming to pending MediaStore Downloads rows, one-transfer/timeout/sequence checks, and partial cleanup; do not use `addJavascriptInterface` or route blob URLs to `DownloadManager`.
- WebUI theme integration accepts only exact-origin, main-frame canonical opaque colors. Its persistent low-overhead observer follows root `class`/`data-skin`, computed `--sidebar`, and head theme-meta replacement/content changes (preferring `#hermes-theme-color`), while native code validates again and resets Android/WebView chrome safely on loading/error/reset.
- Activity recreation may restore a bounded `WebView.saveState` bundle only after configured-origin/current-URL validation. Deep links and notification taps take precedence; reset-session and profile switches must invalidate stale state.
- The background activity toggle owns the lifetime of the existing reconnect foreground service continuously: start/synchronize it while the Activity is foregrounded, retain it across `onStop`/`onResume`, and stop only when disabled or when Android reports explicit task removal through the service's `onTaskRemoved`. Task removal must synchronously latch sticky recovery off, stop reconnect/debug work, clear package notifications, and terminate the app process; do not move that behavior into ordinary Activity or service teardown. Keep its passive ongoing notification on the service-owned low-importance `hermes_background_activity` channel; task/activity alerts remain on `hermes_webui_notifications`. Android 14+ promotion must use the manifest-declared `specialUse` type/property; keep `dataSync` only as the older-version fallback with safe timeout teardown. Do not require a session ID or beta reconnect SSE toggle, use `START_STICKY` recovery only from encrypted settings plus a trusted last URL, retry authenticated `/api/session/stream` with the current CookieManager cookie (startup cookie only as fallback) and bounded cancellation-aware backoff. Do not add boot startup or a second competing service.
...
- OAuth/OIDC code-flow navigations may temporarily load non-allowlisted HTTP/HTTPS provider pages in-app only after Android has parsed an authorization URL whose `redirect_uri` returns to the configured Hermes WebUI origin. Do not broaden this to arbitrary external links or non-web schemes. OAuth provider URLs must never be persisted as `lastLoadedUrl` or restored as the cold-start URL; only configured WebUI routes may be remembered, otherwise the app can cold-start onto a dead provider page with no active OAuth flow.
...
Separate from release publishing, CI uses `.github/workflows/0-ci-build-and-test.yml` to run `clean testDebugUnitTest lintDebug compileDebugAndroidTestKotlin assembleDebug` on pushes/PRs without signing secrets. Keep contributor verification steps aligned with this gate when changing build/test flow. Pin every third-party Action to a full immutable commit SHA with a version comment. Release staging must verify the single expected public signing certificate and publish/check the APK `.sha256` sidecar without clobbering existing release assets.
