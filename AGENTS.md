...
- WebUI theme integration accepts only exact-origin, main-frame canonical opaque colors from `meta[name=theme-color]`, validates again natively, and resets Android/WebView chrome safely on loading/error/reset.
...
- The background activity toggle owns the lifetime of the existing reconnect foreground service while the Activity is backgrounded. Keep its passive ongoing notification on the service-owned low-importance `hermes_background_activity` channel; task/activity alerts remain on `hermes_webui_notifications`. Do not require a session ID or beta reconnect SSE toggle, use `START_STICKY` recovery only from encrypted settings plus a trusted last URL, retry authenticated `/api/session/stream` with the current CookieManager cookie (startup cookie only as fallback) and bounded cancellation-aware backoff, and stop promptly in foreground/when disabled. Do not add boot startup or a second competing service.
